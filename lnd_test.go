...
func assertNumChannelsPending(t *harnessTest, ctxt context.Context,
	...
	if aliceNumChans != expected {
		t.Fatalf("number of pending channels for alice incorrect. expected %v, got %v",
			expected, aliceNumChans)
	}
	if bobNumChans != expected {
		t.Fatalf("number of pending channels for bob incorrect. expected %v, got %v",
...
func testChannelFundingPersistence(net *networkHarness, t *harnessTest) {
	...
	// Restart both nodes to test that they both recover gracefully.
	// TODO(bvu): add restart for Bob.
