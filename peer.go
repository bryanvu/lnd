...
func (p *peer) handleInitClosingSigned(req *closeLinkReq, msg *lnwire.ClosingSigned) {
	...
	respSig := msg.Signature
	responderSig := append(respSig.Serialize(), byte(txscript.SigHashAll))
	closeTx, initiatorSig, err := channel.CompleteCooperativeClose(nil,
		responderSig, proposedFee)
	if err != nil {
		req.err <- err
	...
	parsedInitSig, err := btcec.ParseSignature(initiatorSig, btcec.S256())
	closingSigned := lnwire.NewClosingSigned(chanID, proposedFee,
		parsedInitSig)
...
func (p *peer) handleResponseClosingSigned(msg *lnwire.ClosingSigned,
	...
	// with our signature.
	initSig := msg.Signature
	initiatorSig := append(initSig.Serialize(), byte(txscript.SigHashAll))
	chanPoint := channel.ChannelPoint()

	closeTx, _, err := channel.CompleteCooperativeClose(responderSig,
