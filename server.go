...
func newServer(listenAddrs []string, notifier chainntnfs.ChainNotifier,
	...
	err = sourceNode.ForEachChannel(nil, func(_ *channeldb.ChannelEdgeInfo,
		...
		// if this node is already in the link nodes, check
		// the addresses to see if there's the same address here
		// if so, replace with the port here
		...
		// In case a node has multiple addresses, we'll attempt to
